...
{
public:
	portal(const float& pX, const float& pY, const float& pRadius) : position(glm::vec2(pX, pY)), radius(pRadius), mass(pRadius * 0.1) {}

	void checkCollision(ball& pOther)
	{
		glm::vec2 distanceVect = pOther.position - position;
		float distanceVectMag = glm::length(distanceVect);
		// distance = glm::distance(position, pOther.position);

		if (distanceVectMag < radius + thickness && distanceVectMag > radius - thickness)
		{
			
			float dist_x = pOther.position.x - position.x;
			float dist_y = pOther.position.y - position.y;
...
class ofApp : public ofBaseApp{
	...
	public:
		...
		std::array<portal, 2> portals = { portal(200, 100, 40), portal(300, 300, 40) };
};
